...
class StreamExecutor(AgentExecutor):
    async def execute(self, context: RequestContext, event_queue: EventQueue) -> None:
        ...
        # Emit artifact 1: HTML report
        await asyncio.sleep(0.1)
        html_artifact = Artifact()
        html_artifact.artifact_id = "stream-html"
        html_artifact.name = "Stream Report"
        html_part = html_artifact.parts.add()
        html_part.text = "<html><body><h1>Stream Report</h1><p>Generated by a2a-stream.</p></body></html>"
        html_part.filename = "stream_report.html"
        html_part.media_type = "text/html"

        html_event = TaskArtifactUpdateEvent()
        html_event.task_id = context.task_id
        html_event.context_id = context.context_id
        html_event.artifact.CopyFrom(html_artifact)
        html_event.append = False
        html_event.last_chunk = True
        await event_queue.enqueue_event(html_event)

        # Emit artifact 2: zip file
        await asyncio.sleep(0.1)
        zip_bytes = _make_dummy_zip()
        zip_artifact = Artifact()
        zip_artifact.artifact_id = "stream-zip"
        zip_artifact.name = "Stream Output"
        zip_part = zip_artifact.parts.add()
        zip_part.raw = zip_bytes
        zip_part.filename = "stream_output.zip"
        zip_part.media_type = "application/zip"

        zip_event = TaskArtifactUpdateEvent()
        zip_event.task_id = context.task_id
        zip_event.context_id = context.context_id
        zip_event.artifact.CopyFrom(zip_artifact)
        zip_event.append = False
        zip_event.last_chunk = True
        await event_queue.enqueue_event(zip_event)
...
def make_agent_card() -> AgentCard:
    ...
    card.description = "Streams text word-by-word then emits two file artifacts (HTML + zip) via A2A streaming."
    card.version = "1.2.0"
