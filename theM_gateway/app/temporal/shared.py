...
class OrchestrationInput:
    ...
    # Canonical run_id pre-generated by the caller (Go gateway).
    # When provided, the workflow uses it verbatim so the caller can subscribe to
    # them:dash:run:{run_id}:tokens before starting the workflow.
    # When absent (Python-native callers), the workflow generates one via workflow.uuid4().
