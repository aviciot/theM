...
class OrchestrationWorkflow:
    ...
    async def run(self, inp: OrchestrationInput) -> dict:
        ...
        try:
            ...
            # Use the caller-provided run_id when present (Go gateway pre-generates
            # it so it can subscribe to them:dash:run:{run_id}:tokens before workflow
            # start, avoiding any channel handshake race).
            # Fall back to workflow.uuid4() for Python-native callers.
            canonical_run_id = inp.run_id if inp.run_id else str(workflow.uuid4())
