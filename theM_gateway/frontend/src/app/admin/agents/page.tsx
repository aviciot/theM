    ...
    >
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
        ...
        <div style={{
          ...
          <div>
            <p suppressHydrationWarning style={{ fontSize: '13px', fontWeight: 700, color: 'var(--tm-card-text)', margin: 0, lineHeight: 1, whiteSpace: 'nowrap' }}>
...
export default function AdminAgentsPage() {
  ...
  // ── Folder state ──────────────────────────────────────────────────────────
  // Initialize empty; load from localStorage after mount to avoid SSR/client hydration mismatch.
  const [folderState, setFolderState] = useState<FolderState>({ folders: [] });
  useEffect(() => { setFolderState(loadFolders()); }, []);
            ...
            <Modal wide title="" onClose={() => setScanModal(null)}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '24px' }}>
                ...
                <div>
                  ...
                  <p suppressHydrationWarning style={{ fontSize: '12px', color: 'var(--tm-text-muted)', marginBottom: '20px', lineHeight: 1.5 }}>
                    {timeAgo(result.scanned_at)}<br />
                    <span suppressHydrationWarning style={{ fontSize: '11px', opacity: 0.7 }}>{new Date(result.scanned_at).toLocaleString()}</span>
                  </p>
