...
| **events_transport** | TEXT NOT NULL DEFAULT 'pubsub' | **Phase 11c+** Event delivery transport for this run. `'pubsub'` = legacy Redis Pub/Sub (at-most-once). `'streams'` = Redis Streams with replay. Set to `'streams'` by Go bridge when `RUN_EVENTS_MODE=dual` or `streams` (Phase 11c-B+). Default `'pubsub'` until Go is updated. Added by `db/025_events_transport.sql`. |
...
| `db/025_events_transport.sql` | `them.runs.events_transport TEXT NOT NULL DEFAULT 'pubsub'` column + CHECK constraint (Phase 11c-A) |
