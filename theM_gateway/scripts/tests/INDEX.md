...
| 36 | `run_tests.py::test_36_stream_publish` | structural + unit | no (fakeredis) | Phase 11c-A atomic dual-publish: migration 025 (`events_transport`), `stream_publish.lua` (XADD+PUBLISH+EXPIRE atoms), `TERMINAL_EVENT_TYPES` frozenset, `stream_publish()` helper, all 5 terminal types trigger final TTL, non-terminal types get safety TTL, dual_publish=False skips PUBLISH, Lua script constants (_STREAM_MAXLEN/SAFETY/FINAL_TTL), REDIS.md + SCHEMA.md updated |
| 37 | `run_tests.py::test_37_go_python_parity` | structural | no | Go↔Python migration seam contract: P1 workflow ID `"ctx-"+contextID` in WS+SSE handlers, HITL signal name `submit_human_response`, HITL signal targets `ctx-{context_id}` (DB lookup); P3 channel unification (`them:agents:changed`, `them:token:revoked` publish in `token_cache.py`, `them:ep:config:changed` publish in `_flush_orch_caches`); `epconfig.looksLikeUUID`+`InvalidateApp`; `OrchestrationInput.run_id` field; Python workflow honors caller-provided run_id |
...
| `app/temporal/stream_publish.lua`, `app/temporal/activities.py` (`stream_publish`/`TERMINAL_EVENT_TYPES`), `db/025_events_transport.sql` | 36 |
