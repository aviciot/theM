...
def test_36_stream_publish():
    section("test_36_stream_publish: Phase 11c-A atomic dual-publish via Redis Streams Lua script")

    # ── Migration 025 ─────────────────────────────────────────────────────────
    try:
        m = src("db/025_events_transport.sql")
        check("migration 025 exists", True)
        check("adds events_transport column", "events_transport" in m)
        check("ALTER TABLE them.runs", "them.runs" in m)
        check("NOT NULL DEFAULT 'pubsub'", "NOT NULL" in m and "'pubsub'" in m)
        check("CHECK constraint includes 'streams'", "'streams'" in m)
        check("COMMENT ON COLUMN them.runs.events_transport", "COMMENT ON COLUMN them.runs.events_transport" in m)
    except FileNotFoundError:
        check("migration 025 exists", False, "file not found")
    except Exception as exc:
        check("migration 025", False, str(exc))

    # ── Lua script exists and is well-formed ──────────────────────────────────
    try:
        lua = src("app/temporal/stream_publish.lua")
        check("stream_publish.lua exists", True)
        check("XADD with MAXLEN in Lua", "XADD" in lua and "MAXLEN" in lua)
        check("PUBLISH call in Lua", "PUBLISH" in lua)
        check("EXPIRE for final TTL in Lua", "EXPIRE" in lua and "final_ttl" in lua)
        check("EXPIRE for safety TTL in Lua", "EXPIRE" in lua and "safety_ttl" in lua)
        check("XLEN check for safety TTL guard", "XLEN" in lua)
        check("is_terminal flag used in Lua", "is_terminal" in lua)
        check("dual_publish flag used in Lua", "dual_publish" in lua)
        check("returns entry_id", "return entry_id" in lua)
        check("KEYS[1] = stream_key", "KEYS[1]" in lua)
        check("KEYS[2] = pubsub channel", "KEYS[2]" in lua)
        check("ARGV[4] = is_terminal", "ARGV[4]" in lua)
        check("ARGV[6] = dual_publish", "ARGV[6]" in lua)
    except FileNotFoundError:
        check("stream_publish.lua exists", False, "file not found")
    except Exception as exc:
        check("stream_publish.lua", False, str(exc))

    # ── activities.py structural checks ───────────────────────────────────────
    try:
        a = src("app/temporal/activities.py")
        check("activities.py exists", True)

        # TERMINAL_EVENT_TYPES frozenset
        check("TERMINAL_EVENT_TYPES frozenset defined", "TERMINAL_EVENT_TYPES" in a and "frozenset" in a)
        check("'done' in TERMINAL_EVENT_TYPES", '"done"' in a)
        check("'error' in TERMINAL_EVENT_TYPES", '"error"' in a)
        check("'canceled' in TERMINAL_EVENT_TYPES", '"canceled"' in a)
        check("'terminated' in TERMINAL_EVENT_TYPES", '"terminated"' in a)
        check("'timed_out' in TERMINAL_EVENT_TYPES", '"timed_out"' in a)

        # stream_publish function
        check("stream_publish function defined", "async def stream_publish(" in a)
        check("stream_publish uses register_script", "register_script" in a)
        check("stream_publish reads Lua file", "stream_publish.lua" in a)
        check("stream_publish computes is_terminal from TERMINAL_EVENT_TYPES", "TERMINAL_EVENT_TYPES" in a and "is_terminal" in a)
        check("stream_publish logs entry_id at DEBUG", "logger.debug" in a and "entry_id" in a)
        check("stream_publish does not swallow errors (no bare except: pass)", a.count("except Exception") >= 1)

        # PUBLISH calls replaced with stream_publish
        check("no direct :tokens PUBLISH in activities.py",
              "redis_client.publish" not in a or ":tokens" not in a.split("redis_client.publish")[1] if "redis_client.publish" in a else True)
        check("stream_publish called for token events", 'stream_publish' in a and '"type": "token"' in a)
        check("stream_publish called for terminal events (done/error)", 'stream_publish' in a and '"type": "done"' in a)
        check("_STREAM_MAXLEN constant defined", "_STREAM_MAXLEN" in a)
        check("_STREAM_SAFETY_TTL constant defined (172800)", "_STREAM_SAFETY_TTL" in a and "172800" in a)
        check("_STREAM_FINAL_TTL constant defined (86400)", "_STREAM_FINAL_TTL" in a and "86400" in a)

        # Lua script loaded at module init
        check("Lua script path loaded at module level", "_LUA_SCRIPT_PATH" in a or "_LUA_SCRIPT_SOURCE" in a)
        check("_registered_scripts cache dict defined", "_registered_scripts" in a)

        # _publish_dash still intact (dashboard trace unchanged)
        check("_publish_dash function still present", "async def _publish_dash(" in a)
        check("_publish_dash still publishes to them:dash:run: channel", "_DASH_RUN_PREFIX" in a)

    except FileNotFoundError:
        check("activities.py exists", False, "file not found")
    except Exception as exc:
        check("activities.py structure", False, str(exc))

    # ── Unit tests: Lua script behaviour via fakeredis ────────────────────────
    try:
        import fakeredis
        import fakeredis.aioredis

        async def _run_lua_tests():
            results = []

            # Create fake async Redis client
            fake = fakeredis.aioredis.FakeRedis()

            # Import stream_publish and constants from activities
            import sys
            sys.path.insert(0, str(ROOT))
            # Reload to get a fresh _registered_scripts cache
            import importlib
            import app.temporal.activities as acts
            importlib.reload(acts)

            TERMINAL = acts.TERMINAL_EVENT_TYPES
            sp = acts.stream_publish
            MAXLEN = acts._STREAM_MAXLEN
            SAFETY = acts._STREAM_SAFETY_TTL
            FINAL = acts._STREAM_FINAL_TTL

            run_id = "test-run-001"
            stream_key = f"them:dash:run:{run_id}:stream"
            channel = f"them:dash:run:{run_id}:tokens"

            # ── T1: non-terminal event, first event → XADD + PUBLISH + safety TTL ──
            entry_id = await sp(fake, run_id, {"type": "token", "text": "hello"}, dual_publish=True)
            results.append(("T1a: entry_id returned for non-terminal event", bool(entry_id)))
            xlen = await fake.xlen(stream_key)
            results.append(("T1b: XADD executed (XLEN==1)", xlen == 1))
            ttl = await fake.ttl(stream_key)
            results.append(("T1c: safety TTL set on first event", ttl > 0 and ttl <= SAFETY))

            # ── T2: second non-terminal event → safety TTL NOT reset ──
            ttl_before = await fake.ttl(stream_key)
            await sp(fake, run_id, {"type": "token", "text": "world"}, dual_publish=True)
            ttl_after = await fake.ttl(stream_key)
            results.append(("T2: safety TTL not reset on 2nd event (TTL unchanged or decremented)", ttl_after <= ttl_before))
            xlen2 = await fake.xlen(stream_key)
            results.append(("T2b: XADD executed (XLEN==2 after 2nd event)", xlen2 == 2))

            # ── T3: terminal event → final TTL set ──
            entry_term = await sp(fake, run_id, {"type": "done", "run_id": run_id}, dual_publish=True)
            results.append(("T3a: entry_id returned for terminal event", bool(entry_term)))
            ttl_final = await fake.ttl(stream_key)
            results.append(("T3b: final TTL set on terminal event", ttl_final > 0 and ttl_final <= FINAL))

            # ── T4: dual_publish=False → no PUBLISH (streams-only mode) ──
            run_id2 = "test-run-002"
            stream_key2 = f"them:dash:run:{run_id2}:stream"
            # With fakeredis we can't easily check PUBLISH didn't happen, but we
            # can verify XADD still works and no exception is raised.
            entry_no_pub = await sp(fake, run_id2, {"type": "token", "text": "x"}, dual_publish=False)
            results.append(("T4: stream_publish with dual_publish=False does not raise", bool(entry_no_pub)))
            xlen_no_pub = await fake.xlen(stream_key2)
            results.append(("T4b: XADD still executed when dual_publish=False", xlen_no_pub == 1))

            # ── T5: all 5 terminal types trigger final TTL ──
            for ttype in ["done", "error", "canceled", "terminated", "timed_out"]:
                run_t = f"test-terminal-{ttype}"
                sk = f"them:dash:run:{run_t}:stream"
                await sp(fake, run_t, {"type": ttype}, dual_publish=False)
                ttl_t = await fake.ttl(sk)
                results.append((f"T5: '{ttype}' triggers final TTL ({FINAL}s)", ttl_t > 0 and ttl_t <= FINAL))

            # ── T6: non-terminal types do NOT set final TTL ──
            for ntype in ["token", "plan_done", "tool_start", "tool_done", "usage", "agent_status", "iteration_start"]:
                run_n = f"test-nonterminal-{ntype}"
                sk_n = f"them:dash:run:{run_n}:stream"
                await sp(fake, run_n, {"type": ntype}, dual_publish=False)
                ttl_n = await fake.ttl(sk_n)
                # Safety TTL is set on first event — it must be <= SAFETY, not <= FINAL
                # (FINAL < SAFETY, so if TTL <= FINAL it would be the wrong TTL)
                results.append((f"T6: '{ntype}' gets safety TTL not final TTL", FINAL < ttl_n <= SAFETY))

            # ── T7: TERMINAL_EVENT_TYPES contains exactly the 5 expected types ──
        ...
        test_results = asyncio.run(_run_lua_tests())
        for desc, ok in test_results:
            check(desc, ok)

    except ModuleNotFoundError as exc:
        skip(f"stream_publish unit tests — missing deps (run inside container): {exc}")
    except Exception as exc:
    ...
    try:
        rd = src("docs/REDIS.md")
        check("them:dash:run:{runID}:stream key documented in REDIS.md", "them:dash:run:" in rd and ":stream" in rd)
        check("MAXLEN documented in REDIS.md", "MAXLEN" in rd or "5000" in rd)
        check("safety TTL documented in REDIS.md", "48h" in rd or "172800" in rd or "safety" in rd.lower())
        check("final TTL documented in REDIS.md", "24h" in rd or "86400" in rd or "final" in rd.lower())
    except Exception as exc:
        check("REDIS.md stream key", False, str(exc))

    # ── SCHEMA.md updated ─────────────────────────────────────────────────────
    try:
        sm = src("docs/SCHEMA.md")
        check("events_transport column documented in SCHEMA.md", "events_transport" in sm)
    except Exception as exc:
