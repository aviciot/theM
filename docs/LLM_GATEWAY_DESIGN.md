...
**Phase 1 — gateway, observe + meter. ✅ COMPLETE (commit `4a9402e7`, 2026-09-19).** Migration `db/100_llm_gateway.sql` (6 tables). New `internal/llmgateway` (handler → service → DAL). `POST /{tenant_slug}/llm/v1/chat/completions`, streaming and non-streaming. Bearer auth, tenant match, quota via existing `Enforcer`, usage recorded in `defer`. 25 tests (S1-119). `go test ./...` — 0 failures.

**Phase 2 — governance. ✅ COMPLETE (commit `936ae696`, 2026-09-19).** `gateway_policies` enforced: model aliases resolved first, allowed_models allowlist (→ 403 `ErrModelBlocked`), `max_tokens_per_request` ceiling applied to Build call, `monthly_budget_usd` checked via `SumMonthlySpend` (→ 429 `ErrBudgetExceeded`). All checks fail-open on DB error (§16.2). No new tables (table existed from Phase 1). 8 tests (S1-120). `go test ./...` — 1287 pass, 0 fail. **Decision settled:** gateway USD budget and run `monthly_llm_tokens` are independent — a closed agent's spend cannot exhaust hosted-app token quotas.
