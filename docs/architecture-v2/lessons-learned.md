...
## Platform and Integration Lessons (L-01 through L-09)

*Added 2026-07-25 — consolidated from go/docs/architecture-v2/lessons-learned.md (2026-07-20). These cover Windows/Docker platform traps, cross-bridge behavioral differences, and DAL test patterns.*
...
dashes `—` in log messages) cause the terminal to crash or corrupt output.
...
mangled by MSYS path translation, causing `docker cp` to fail or copy to the wrong path.

**Fix:** Replace `docker cp + psql -f` with `docker exec -i container psql ... < file`.
Git Bash handles the `< file` stdin redirect natively without path mangling.
...
columns in later migrations, the original soak seed SQL became inconsistent.

**Fix:**
1. Always run `\d them.runs` before writing seed SQL to confirm the current schema.
2. The seed SQL uses `ON CONFLICT DO NOTHING / DO UPDATE` for idempotency — the column
   list in the INSERT must match exactly.
3. Add a pre-check: if `them.runs` exists, skip migrations and go directly to seeding.
...
not yet ready to serve namespace queries. The CLI returns a gRPC error, Docker marks the
service unhealthy, and dependent services (Go bridges) fail to start.
...
```
The Go bridges have their own retry logic on Temporal client connect and do not require
Docker-level healthcheck to pass first.
...
The config loader uses `getEnvBoolSafe` where `safeDefault=true`. Any unset, empty, or
invalid value returns `true`, never `false`. This means a misconfigured deployment cannot
accidentally enable writes.

Note: `strconv.ParseBool` accepts `"1"`, `"t"`, `"T"`, `"TRUE"`, `"true"`, `"True"`,
`"0"`, `"f"`, `"F"`, `"FALSE"`, `"false"`, `"False"` — only truly invalid strings
(e.g., `"yes"`, `"no"`) fall back to the safe default.
...
L1+L2 cache, but the admin path never holds the raw plaintext after creation.

**What happened:** `auth.Cache.Revoke(ctx, rawToken)` hashes the token internally and
cannot be called from the admin service which only has `token_hash`.

**Fix:** `TokenService.invalidate(ctx, hash)` bypasses `Revoke` and calls cache
primitives directly:
...
**Context:** Older Python test scripts that compared raw string output fail when
pointing at the Go bridge.

**What happened:** `python3 -c "print(d.get('enabled'))"` prints `True` (Python bool
repr), not JSON `true`. Bash-comparison scripts that check for `True` fail against Go.

**Fix:** The Python↔Go contract tests in `test_go_wave5_contracts.py` parse JSON
responses and compare Python objects, never raw strings. When writing any new
cross-bridge test, always use `json.loads()` and compare Python types.
...
### L-11: Fernet compatibility — Python key derivation returns raw bytes; base64 is transport only

**Context:** Wave 7 Phase 1 — porting Python's `app/utils/crypto.py` Fernet encryption to Go.

**What happened:** Python stores the Fernet key as `base64url_encode(sha256(SECRET_KEY))` and
passes that base64url string to `Fernet(key)`. A naive Go port might think the "key" is a
44-char base64 string and try to derive or compare keys at that level. In reality, the Fernet
library immediately base64-decodes the key string back to 32 raw bytes. The 44-char base64url
string is purely a transport encoding for the `Fernet()` constructor; the real key material is
the 32 SHA-256 bytes.

**Fix:** Go's `DeriveKey(secretKey)` returns `sha256.Sum256([]byte(secretKey))[:]` — 32 raw
bytes — with no intermediate base64 encoding. The Go package works directly with these 32 bytes
for key splitting (`signing_key = raw[0:16]`, `encryption_key = raw[16:32]`). This is
byte-for-byte identical to what Python's Fernet library uses internally.

**Proven bidirectional compatibility:** A static known-vector token generated by Python's
`cryptography.fernet.Fernet` (with fixed IV + timestamp for determinism) is verified by
`Python Fernet.decrypt()` AND decrypted correctly by Go's `Decrypt()` (Direction 1). Go's
`Encrypt()` + manual algorithm replay in Python confirms Direction 2 (Go encrypts → Python
decrypts). Known vector is embedded in `go/internal/crypto/fernet_test.go`.

**Rule:** When porting any Python `cryptography` library usage to Go: find out what format
the library actually uses internally (read the source or docs), not what format it exposes to
callers. The constructor API and the internal representation are often different.

---

### L-12: Fernet HMAC covers Version+Timestamp+IV+Ciphertext — verify BEFORE decryption

**Context:** Wave 7 Phase 1 — Fernet manual implementation security ordering.

**Rule:** The Fernet spec requires HMAC verification **before** any AES decryption step. This
prevents padding oracle attacks: an attacker who can manipulate ciphertext and observe decryption
errors can recover key material one byte at a time if HMAC is checked after decryption.

**Implementation:** `crypto/subtle.ConstantTimeCompare(storedMAC, expectedMAC)` runs on the
full `Version+Timestamp+IV+Ciphertext` region before `cipher.NewCBCDecrypter` is ever called.
A bad HMAC returns `ErrHMACMismatch` immediately. Python's `cryptography.fernet` also enforces
this ordering (required by the spec).

**Do not reorder:** If you ever refactor `Decrypt()` in `go/internal/crypto/fernet.go`, ensure
the HMAC check precedes the `aes.NewCipher` call. Moving it after AES decryption for "convenience"
is a security regression even if all other tests pass.

---

### L-13: "enc:" prefix passthrough for legacy unencrypted values — match Python exactly

**Context:** Wave 7 Phase 1 — `DecryptStored` must handle DB values that pre-date encryption.

**What happened:** Some rows in `*_encrypted` columns were written before encryption was
introduced. They contain plaintext values (no `"enc:"` prefix). Python's `decrypt_value()`
returns the value unchanged when the prefix is absent. Go's `DecryptStored` must do the same.

**Fix:** `if stored == "" || !hasPrefix(stored, storedPrefix) { return stored, nil }` — the
value is returned as-is, no error. This handles: empty string (no API key configured), legacy
plaintext values, and NULL-equivalent DB reads.

**Critical distinction:** "no prefix → passthrough" is a compatibility path, NOT a security
feature. Any new code that stores an API key must always call `EncryptStored` (which adds the
prefix). Only rows that already exist in production without the prefix benefit from this path.
