...
```
1. Client: WS connect to ws://them-bridge:8001/ws/orchestrate/my-orchestrator
           Authorization: Bearer <opaque_token>

2. Bridge: validate_token(token)
   a. L1 cache hit → use cached user_context
   b. L1 miss → check Redis them:session:token:{sha256(token)} (TTL 300s)
   c. Redis miss → SELECT from them.access_tokens WHERE token_hash=... AND enabled
   d. Cache result in L2 Redis + L1 in-process

3. Bridge: load_orchestrator(name)
   a. Check Redis them:orchestrators:{name} (TTL 600s)
   b. Miss → SELECT from them.orchestrators WHERE name=... AND enabled
...
5. Bridge: rate_limit check → Redis INCR rl:them:{user_id}:{hour_slot}

6. Bridge: run_recorder.start_run() → INSERT them.runs (status=running)
...
9. Orchestrator loop (iteration 1..max_iterations):
   ...
   f. Each adapter call → run_recorder.record_step() them.run_steps
   g. Collect results → feed to LLM as tool_results
   h. run_recorder.record_usage() → them.run_usage

10. LLM emits final answer (no more tool calls)
    → stream remaining tokens to client
    → {"type": "done", "run_id": "..."}

11. run_recorder.complete_run() → UPDATE them.runs status=completed
```
...
Bridge publishes to Redis pub/sub them:dash:{channel}
