...
| `them:dash:run:{run_id}:stream` | MAXLEN ~5000 entries (no time TTL) | go/internal/runstream/publisher.go (`PublishEvent`), go/internal/appflow/activities.go (direct `XAdd`) | Yes | Redis Stream — durable, replayable per-run event log consumed by WS/SSE (`internal/ws`, `internal/sse`) via `StreamFromRedis`, which replays from a client's `last_event_id` on reconnect and emits a synthetic `replay_unavailable` event if the cursor aged out of the MAXLEN window. Single field `data` → JSON object with a required `type` key. Known types: `token`, `tool_call`, `tool_result`, `done`, `error`, `replay_unavailable`, `file`, and (Phase 2 of `docs/APP_CANVAS_DEBUG_PLAN.md`) `node_start`/`node_done`/`node_error` — `{type, run_id, node_id, kind, detail?}`, emitted unconditionally by every AppFlow node kind (Router/HIL/Agent/Inline LLM inline from their own activities; Condition/Fork/Join via the new `AppFlowTraceNodeEventActivity`), not yet persisted anywhere (Phase 3). Phase 6 adds `node_paused` (same wire shape) — fired by `stepGate` right before a node blocks waiting for the debug Step signal (`AppFlowSignalStep`); transient, never persisted to `them.run_steps` (no lifecycle-state slot for "waiting" in `persistTrace`'s insert-then-update model), live-publish only. Unknown `type` values are silently ignored by existing WS/SSE consumers (fail-open), so new types are safe to add without a wire-format migration. **Also tailed by `internal/dashboard`'s `/ws/dashboard` handler** (`tailRunChannel`, Phase 5 of `docs/APP_CANVAS_DEBUG_PLAN.md`) for any client subscribing to a `run:{run_id}` channel — reuses `StreamFromRedis` (replay + live XREAD BLOCK), the same primitive `internal/ws`/`internal/sse` use. Fixes a real pre-existing gap found while building the App Canvas Debug Mode Phase 5 frontend: nothing ever `PUBLISH`es these events (only `XADD`), so a plain pub/sub subscribe on a `run:*` channel previously delivered only a one-shot `XRevRange` snapshot taken at subscribe time — any run fast enough to finish before that snapshot's Redis round-trip (e.g. mock-LLM AppFlow debug runs, often <1s end-to-end) got zero events delivered live. This also fixes the same latent gap in the playground's own `run:*` trace pane (`useChatConnection.ts`'s `openDashWs`), not just the new debug panel. **This channel's subscribe path also gained a tenant-ownership check this session** (`dashboard.RunOwnershipChecker`) — previously any authenticated caller of any tenant could subscribe to any other tenant's `run:{id}` and read its live trace; see `docs/LESSONS.md`'s entry on this. |
| `them:debug:{tenant_id}:{run_id}:{node_id}:llm_override` | 3h40m (`debugcred.TTL` = `appflow.DebugRunMaxLifetime` (3h30m) + `debugcred.CleanupMargin` (10min)) — derived FROM the same enforced ceiling `Lifecycle.StartAppFlow` sets as the debug workflow's `WorkflowRunTimeout`, never an independently guessed number, so "the run is still legitimately active" and "the credential hasn't expired" can never disagree; see both constants' own doc comments | go/internal/admin/service/appflow_debug.go (`AppFlowDebugService.Start`, via `go/internal/debugcred.Store.Set`) | Yes | Per-node LLM credential override for one App Canvas Debug Mode run only (`docs/APPFLOW_RUNTIME_PARAMS_PLAN.md`) — never applied to production runs, never written to the app's saved Runtime settings (`them.app_flow_llm_overrides`/`flow-llm-nodes` API). Value: JSON `{user_id, provider, model, api_key, base_url, marker_only}`. Read by `go/cmd/dag-worker/main.go`'s `dbLLMCaller.Complete` — a plain non-destructive `GET` (survives repeated reads across a Temporal activity retry), checked only when the activity's `Debug` flag is set. One entry is written for every canvas node with a declared `llm_credential` runtime param, even at "use tenant default," so a reader can tell "requested but now gone" (must fail loudly) apart from "never requested" (falls through to normal resolution) purely by whether the key exists. Concurrency-safe by construction — `run_id` is a fresh UUIDv4 per debug/start call, so no two runs (same tenant or not) ever share a key. **Deleted proactively on run completion** via `Store.DeleteAllForRun` (cursor-based `SCAN`+`DEL`, never `KEYS`), called from `AppFlowActivities.FinalizeRunActivity` — the single hook every AppFlow run already calls on every exit path, gated on the run's `Debug` flag. TTL is strictly the fallback for a run abandoned, crashed, or killed by the workflow's own `WorkflowRunTimeout` before reaching a terminal state. |
