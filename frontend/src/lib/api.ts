...
export const themApi = {
  ...
  getProviderKeys: (appId: string) => api.get<{ provider: string; key_set: boolean; key_hint?: string; base_url?: string }[]>(`/admin/applications/${appId}/provider-keys`),
  setProviderKey: (appId: string, provider: string, key: string, baseURL?: string) => api.put<{ provider: string; updated: boolean }>(`/admin/applications/${appId}/provider-keys/${provider}`, { key, base_url: baseURL ?? '' }),
  ...
  getTenantResources: (id: string) =>
    api.get<{ applications: number; agents: number; users: number }>('/admin/tenants/' + id + '/resources'),
  deleteTenant: (id: string, force = false) =>
    api.delete<void>('/admin/tenants/' + id + (force ? '?force=true' : '')),
