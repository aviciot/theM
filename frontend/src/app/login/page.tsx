          ...
          <div className="mt-5">
            {!showOrgCode ? (
              <div className="flex items-center gap-3">
                ...
                <button type="button" onClick={() => setShowOrgCode(true)}
                  className="text-xs text-slate-500 hover:text-slate-400 transition-colors whitespace-nowrap flex items-center gap-1.5">
              ...
              <div className="rounded-xl p-4 space-y-3"
                style={{ background: 'rgba(15,17,23,.7)', backdropFilter: 'blur(20px)', border: '1px solid rgba(255,255,255,.08)', boxShadow: '0 8px 24px rgba(0,0,0,.4)' }}>
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <svg className="w-4 h-4 text-blue-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-2 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" />
                    </svg>
                    <span className="text-sm font-semibold text-slate-300">Sign in with SSO</span>
                  </div>
                  <button type="button" onClick={() => { setShowOrgCode(false); setOrgCode(''); }}
                ...
                <div className="relative">
                  <input
                    ...
                    onKeyDown={e => { if (e.key === 'Enter') handleOrgCodeSSO(); }}
                    placeholder="your-organization"
                    autoFocus
                    className="brand-input" style={{ paddingLeft: '12px' }}
                  />
                </div>
                <p className="text-[11px] text-slate-500">Enter your organization's SSO identifier</p>
                <button type="button" onClick={handleOrgCodeSSO} disabled={!orgCode.trim()}
                  className="w-full flex items-center justify-center gap-2 py-2.5 px-4 rounded-lg text-sm font-bold text-white transition-all active:scale-[0.98] disabled:opacity-40"
                  style={{ background: 'linear-gradient(135deg, #2563eb 0%, #4338ca 100%)', boxShadow: '0 4px 15px rgba(59,77,255,.3)' }}>
                  Continue with SSO
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
