...
export function RuntimeView({ app, onBack }: { app: Application; onBack: () => void }) {
  ...
  useEffect(() => {
    themApi.listEntryPoints(app.id).then(eps => {
      ...
      eps.forEach(ep => {
        const hw = ep.history_window ?? 20; sd[ep.id] = { historyEnabled: hw > 0, memoryEnabled: ep.memory_enabled ?? false, historyWindow: hw > 0 ? hw : 20, summarizeEveryN: ep.summarize_every_n_calls ?? 10, fallbackN: ep.memory_raw_fallback_n ?? 3, provider: ep.summarizer_provider ?? '', model: ep.summarizer_model ?? '' };
