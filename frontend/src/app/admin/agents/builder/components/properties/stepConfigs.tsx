...
interface BaseArgs {
  ...
  cfgNum: (key: string, def?: number) => number;
...
export function renderInputConfig({ cfg, cfgStr, updateStepConfig }: InputArgs): ReactElement {
  const bindingsText = (cfg.bindings as Record<string, string>)?.text ?? '';
  return (
    <>
      ...
      <input
        value={cfgStr('text_var') || bindingsText}
        onChange={e => updateStepConfig('bindings', { text: e.target.value })}
        style={inputStyle}
        placeholder="e.g. user_query"
      />
      <div style={{ marginTop: 6, fontSize: 11, color: '#64748b' }}>
        The caller&apos;s message text will be available as <code style={{ color: C.cyan }}>{'{{.' + (cfgStr('text_var') || bindingsText || 'user_query') + '}}'}</code> in downstream steps.
      </div>
    </>
  );
}

interface LlmArgs extends BaseArgs {
  mcpServers: MCPServer[];
  cfg: Record<string, unknown>;
}

export function renderLlmConfig({ cfgStr, cfgNum, updateStepConfig, mcpServers, cfg }: LlmArgs): ReactElement {
...
interface HttpArgs extends BaseArgs {
  cfg: Record<string, unknown>;
}

export function renderHttpConfig({ cfgStr, cfgNum, cfg, updateStepConfig }: HttpArgs): ReactElement {
  return (
    <>
...
export function renderTransformConfig({ selectedNodeId, localPipeNodes, localPipeEdges, cfg, updateStepConfig }: TransformArgs): ReactElement {
  ...
  return <TransformPanel cfg={cfg} updateStepConfig={updateStepConfig} availableVars={availableVars} />;
}

export function renderResponseConfig({ cfgStr, updateStepConfig }: Pick<BaseArgs, 'cfgStr' | 'updateStepConfig'>): ReactElement {
  return (
    <>
...
export function renderBranchConfig({ cfgStr, updateStepConfig }: Pick<BaseArgs, 'cfgStr' | 'updateStepConfig'>): ReactElement {
  return (
    <>
...
export function renderLoopConfig({ cfgStr, cfgNum, updateStepConfig }: BaseArgs): ReactElement {
  return (
    <>
