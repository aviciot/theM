...
// (single-tenant / bootstrap use).
//
// Isolation invariant:
//   - externalUserID != "": filter to that end-user's tasks only (backend-asserted identity)
//   - userID != 0: filter to that the-M user's tasks only (AccessModeUser / internal session)
//   - both empty/zero: no user filter (anonymous or public EP sessions)
//
// Legacy rows (both user_id and external_user_id NULL) are excluded when either
// filter is active, because NULL != any non-NULL value in SQL.
func (s *Store) LoadHistory(ctx context.Context, contextID, tenantID, externalUserID string, userID int64, limit int) ([]domain.Message, error) {
	const q = `
...
LIMIT $5`
	...
	rows, err := s.pool.Query(ctx, q, contextID, tenantID, externalUserID, userID, limit)
	if err != nil {
...
// then assigns the next sequence number. externalUserID and userID are stored on
// the task row to scope history reads per identity.
func (s *Store) WriteMessage(ctx context.Context, contextID, runID, tenantID, externalUserID string, userID int64, msg domain.Message) error {
	taskID, err := s.resolveRootTaskID(ctx, contextID, runID, tenantID, externalUserID, userID)
	if err != nil {
...
// Returns "" with nil error when no summary exists. Applies the same user-scoping
// as LoadHistory: externalUserID filter takes precedence over userID filter.
func (s *Store) LoadSummary(ctx context.Context, contextID, tenantID, externalUserID string, userID int64) (string, error) {
	const q = `
SELECT tm.parts
FROM them.task_messages tm
JOIN them.tasks t ON t.id = tm.task_id
...
LIMIT 1`
	...
	row := s.pool.QueryRow(ctx, q, contextID, tenantID, externalUserID, userID)
	var raw []byte
...
func (s *Store) SaveSummary(ctx context.Context, contextID, runID, tenantID, externalUserID string, userID int64, summary string) error {
	msg := domain.Message{
...
LIMIT 1`
	...
	// user_id is nullable — NULL for anonymous/external-only/public EP sessions.
	const insertQ = `
...
RETURNING id::text`
	row = s.pool.QueryRow(ctx, insertQ, contextID, runID, tenantID, externalUserID, userID)
	if err := row.Scan(&id); err != nil {
