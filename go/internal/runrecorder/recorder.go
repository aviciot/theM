...
func (r *Recorder) CreateRun(ctx context.Context, run domain.Run) error {
	...
	const q = `
		INSERT INTO them.runs (id, tenant_id, entry_point_slug, status, started_at, events_transport, goal, orchestrator_name, external_user_id, user_id)
		VALUES ($1, $2::uuid, $3, $4, $5, $6, NULLIF($7,''), NULLIF($8,''), NULLIF($9,''), NULLIF($10,0)::integer)
	...
	err := r.db.Exec(ctx, q,
		...
		run.Goal, run.OrchestratorName, run.ExternalUserID, run.UserID,
	)
	if err != nil {
