...
func TestTranscribe_EPNotFound(t *testing.T) {
	...
	req, _ := http.NewRequest(http.MethodPost, srv.URL+"/default/apps/my-app/my-voice-ep/voice/transcribe", buf)
...
func TestTranscribe_TokenEPNoAuth(t *testing.T) {
	...
	buf, ct := buildMultipart(t, []byte("audio"))
	req, _ := http.NewRequest(http.MethodPost, srv.URL+"/default/apps/my-app/my-ep/voice/transcribe", buf)
	req.Header.Set("Content-Type", ct)
	resp, err := http.DefaultClient.Do(req)
	if err != nil {
		t.Fatal(err)
...
func TestTTS_MissingText(t *testing.T) {
	...
	req, _ := http.NewRequest(http.MethodPost, srv.URL+"/default/apps/my-app/my-ep/voice/tts", bytes.NewReader(body))
