...
| `TestGuardHealth_Handler_Empty` | GH-H1: GET /applications/{id}/guard-health, no wirings/scans → 200 with empty agents and zero stats |
| `TestGuardHealth_Handler_WithAgent` | GH-H2: GET /applications/{id}/guard-health with one wiring → 200 with agent in agents array |
...
### S1-169 · A2A response part-kind decoding — `cmd/dag-worker/agent_response_kinds_test.go`

**Purpose:** Phase 1 of `docs/APPFLOW_A2A_RESPONSE_KINDS_PLAN.md`. Before this
change, `pgxAgentA2ACaller.InvokeByID` decoded an agent's A2A SendMessage
response into a hand-rolled struct with only a `Text string` field — a
file/data/raw part (all 3 real, valid A2A response shapes) silently decoded
to an empty string, no error, nothing logged. The decode+classify logic was
extracted into a standalone `decodeAgentSendMessageResponse(io.Reader)`
function (no DB/HTTP dependency) specifically so it's unit-testable —
`InvokeByID` itself still isn't (needs a live Postgres pool + HTTP server).

| Test | What it proves |
|---|---|
| `TestDecodeAgentSendMessageResponse_TextPart` | a plain text part resolves to `ResponseText`, `PartKind` empty — the pre-existing, overwhelmingly common case is completely unaffected |
| `TestDecodeAgentSendMessageResponse_FilePart` | a file (`url`) part is now recognized: `PartKind="file"`, `FileURL`/`FileName`/`FileContentType` all populated — previously silently dropped |
| `TestDecodeAgentSendMessageResponse_DataPart` | a structured `data` part is recognized (`PartKind="data"`) — not yet given special handling beyond the marker, per the plan's explicit scope |
| `TestDecodeAgentSendMessageResponse_RawPart` | a `raw` bytes part is recognized (`PartKind="raw"`) |
| `TestDecodeAgentSendMessageResponse_RPCError` | a JSON-RPC error response still surfaces as a Go error, unaffected by the part-decoding change |
| `TestDecodeAgentSendMessageResponse_NoArtifacts` | an empty artifacts list resolves to a zero-value result, no error — matches prior "agent said nothing" behavior |
| `TestDecodeAgentSendMessageResponse_TextTakesPriorityWhenFirst` | when a part carries text, it's returned immediately — the documented priority order (text > file > data > raw) is honored, not "last part wins" |

**Trigger:** any change to `cmd/dag-worker/main.go`'s `decodeAgentSendMessageResponse`/`pgxAgentA2ACaller.InvokeByID`, or `go/vendor/github.com/a2aproject/a2a-go/v2/a2a/core.go`'s `Part` type
...
| `TestTenants_AddMember_QuotaNilLimit_Allows` (TN-23) | max_users=nil → no enforcement → 201 |
| `TestTenants_AddMember_QuotaUnderLimit_Allows` (TN-24) | max_users=5, count=4 → 201 |
| `TestTenants_AddMember_QuotaAtLimit_Rejects` (TN-25) | max_users=5, count=5 → 429 |
...
### S1-100 · Audit Logs handler — `internal/admin/audit_logs_test.go`, `internal/admin/mcp_servers_audit_test.go`

**Purpose:** `GET /admin/audit-logs` returns tenant-scoped audit log rows with correct fields; pagination params accepted. MCP server audit path is nil-safe.
...
**Purpose:** `GET /admin/observability/summary` returns a cross-tenant aggregate JSON array using the Admin (BYPASSRLS) pool, with 30d DB data merged with live-today Redis numbers (runs_today, active_users_today, is_live) per tenant; `GET /admin/observability/tenant/{id}/apps` returns per-app 30d DB stats merged with Redis live-today numbers; DB errors return 500. AppIDs are fetched in a single query via ARRAY_AGG and used to sum per-app Redis keys without extra DB round-trips.
...
| OBS-9 | `TestObservability_Summary_LiveFieldsPresent` | runs_today, active_users_today, is_live present in response; app_ids absent from JSON (json:"-") |
...
**Trigger:** any change to `internal/admin/middleware_wirings.go` (ListDefs handler), `internal/admin/dal/middleware_wirings.go` (ListMiddlewareDefs / MiddlewareDefSummary), `internal/admin/node_types.go` (`middlewareNodeInfo`, merges this DAL's output into `/admin/node-types` as the "middleware" family — see S1-126), or `db/097_middleware_defs_visual.sql` / `db/102_middleware_defs_node_contract.sql`
...
| AF-WF-17 | `TestInvokeAgentActivity_CarriesFilePartThrough` | a recognized file-kind AgentInvokeResult (PartKind/FileURL/FileName/FileContentType) carries through to AgentInvokeActivityOutput unchanged; node_done trace detail shows a non-blank `[file response, no text]` marker instead of an empty string (docs/APPFLOW_A2A_RESPONSE_KINDS_PLAN.md Phase 1) |
| AF-WF-18 | `TestFileGateActivity_NilGate_NoOp` | FileGateActivity with a nil FileGate dependency is a safe no-op (ScanStatus="disabled") — docs/APPFLOW_A2A_RESPONSE_KINDS_PLAN.md Phase 2 |
| AF-WF-19 | `TestFileGateActivity_DelegatesToGate` | FileGateActivity delegates to the configured FileGate and passes NodeID through unchanged — the scoping key gate.go's node_id fix (S1-170) actually uses |
| AF-WF-20 | `TestFileGateActivity_GateError_Propagates` | a real gate error (e.g. storage unavailable) propagates wrapped with node context, never silently looks like "scanning disabled" |
...
| `internal/admin/system_agent_resolve.go` (`resolveSystemAgentRole`) | S1-142 |
| `internal/admin/classify.go` (`classifyAgent`) | S1-143 |
| `internal/admin/synthesize.go` (`synthesizeAppCard`, `dispatchLLMText`) | S1-144 |
| `internal/admin/service/tenant_system_agent_config.go` | S1-145 |
| `internal/admin/tenant_system_agent_config.go` (`TenantSystemAgentConfigHandler`) | S1-146 + S1-148 |
| `internal/admin/service/tenant_system_agent_config.go` (`ResolveCustomTestInputs`) | S1-147 |
| `internal/admin/dal/tenant_system_agent_config.go`, `db/106_tenant_system_agent_config.sql` | S2-13 (integration) |
| `internal/admin/security_scan_llm.go` (`llmCardAnalysis`, `mergeSecurityScanResult`), `internal/admin/scanjob.go` (`runScanJob` merge step) | S1-150 — and rebuild + restart `them-security-agent` (Python probes-only scanner, `agents/security_scanner/scanner.py`, no longer runs the LLM step) |
| `internal/admin/dal/llm_provider_keys.go` (`tenant_id *string` widened, db/108), `db/108_platform_llm_provider_keys.sql` | `go test -tags=integration ./internal/admin/dal/...` (ex-S2-14, 4 platform-owned-key tests removed by Platform-as-Tenant Phase 2, 2026-09-24 — no live caller ever passes `TenantID: nil` anymore, see the S2 "removed" row) — `internal/admin/llm_provider_keys_platform.go` (former platform routes, ex-S1-151) also deleted; the `TenantScopedRoutes` half in `llm_provider_keys.go` is unaffected |
| `internal/admin/dal/llm_providers.go` (`ListProvidersForTenant`, bootstrap-tenant fallback), `internal/admin/dal/app_config.go` (`GetProviderBaseURLs`, bootstrap-tenant fallback) | S2-17 (integration) — Platform-as-Tenant Phase 2, 2026-09-24 |
| `db/110_platform_as_bootstrap_tenant.sql` RLS policies (`llm_providers_read`, `llm_providers_write`, `llm_provider_keys_tenant_isolation`) | S2-18 (integration, `internal/db`) — Platform-as-Tenant Phase 3, 2026-09-24 |
| `internal/admin/service/appflow_debug_presets.go`, `internal/admin/appflow_debug_presets.go`, `db/111_appflow_debug_presets.sql` | S1-160 |
| `internal/admin/service/llm_providers.go` (`UpsertForTenant`'s bootstrap-tenant template lookup) | `service/llm_providers_test.go` `TestTenantProvider_Upsert_*` — Platform-as-Tenant Phase 2, 2026-09-24; ex-`GetPlatformProviderRow`/ex-S1-153 deleted, no replacement (function removed, not relocated) |
| `internal/admin/agents.go` (`Discover` tenantID hoist), `internal/admin/ep_discover.go` (`synthesizeAppCard` call site) | S1-143 + S1-144 (call-site behavior covered via the function-level tests, not a dedicated handler-level test) |
...
| S1-101 | Observability summary + per-app breakdown (OBS-1..9): Summary_OK, Summary_Empty, Summary_DBError, Summary_MultiTenant, AppBreakdown_OK, AppBreakdown_Empty, AppBreakdown_DBError, AppBreakdown_MultiApp, Summary_LiveFieldsPresent | 9 |
...
| S1-130 | App Canvas Debug Mode Phase 2 — activity-level node_start/node_done/node_error trace emission (`workflow_test.go`, AF-TR-01..07): `TestInlineLLMActivity_EmitsStartAndDoneTrace`, `TestInlineLLMActivity_EmitsErrorTrace`, `TestInvokeAgentActivity_EmitsStartAndDoneTrace`, `TestExecuteHILActivity_EmitsStartThenErrorTrace_NilDB` (node_start always fires; node_done for HIL fires later from `execHILNode`, not this activity), `TestExecuteRouterActivity_EmitsStartAndDoneTrace` (detail=`label=<chosen>`), `TestTraceNodeEventActivity_PublishesEvent`, `TestTraceNodeEventActivity_NilStreamPub_NoOp`. Also updated `TestInlineLLMActivity_StreamPublishesToken` to filter for the `"token"`-type payload specifically, since node_start/node_done now also publish to the same stream. **Platform-as-Tenant Phase 6 fix (2026-09-24):** `TestInlineLLMActivity_EmitsStartAndDoneTrace`/`TestInvokeAgentActivity_EmitsStartAndDoneTrace` strengthened to assert `node_done`'s `detail` field equals the LLM/agent's real response text — found live: `InlineLLMActivity`/`InvokeAgentActivity` (`activities.go`) both hardcoded `detail=""` on their `node_done` emitTrace call, discarding `responseText`/`text` even though both were already computed (and, for LLM, already used a few lines later for token streaming). Every other node kind (router, condition, fork/join) already put its real output into `detail` — LLM and agent were the two outliers. The App Canvas Debug Mode inspector's "No output captured for this node yet." message for a `done` LLM node was the live symptom. | 0 net new (2 existing tests strengthened, no new test count) |
...
| **gap (465ffe93)** | Tenant LLM Provider Keys DAL+service layer (commit `465ffe93`) added `internal/admin/service/llm_provider_keys_test.go` (18 tests), `internal/admin/dal/llm_provider_keys_integration_test.go` (9 integration tests, tag `integration`), plus `allowed_models` round-trip cases appended to `internal/admin/service/llm_providers_test.go` (already counted incrementally as this file grew — no separate row was ever added for that commit's net-new tests). Not reconciled here — flagged so the count is understood to already run ahead of the row-by-row total below; full backfill not in scope for this change. | not counted below |
| S1-135 | Tenant LLM Provider Keys — HTTP endpoints (this session): `LLMProviderService.GetOwnProviderRow` (`llm_providers_test.go`) — `TestGetOwnProviderRow_NotFound_ReturnsErrNotFound` (tenant has no own row for that provider name → ErrNotFound, never falls back to the platform row), `TestGetOwnProviderRow_Found_ReturnsTenantRow_NeverPlatformID` (returns the tenant's own row id, confirms it is not the platform row's id even when both exist for the same name) | 2 |
| S1-136 | Tenant LLM Provider Keys — live model-list fetch helpers (`llm_provider_models_test.go`, new `internal/admin/llm_provider_models.go`): `TestDoModelsFetch_Success_DecodesBody`, `TestDoModelsFetch_NonOK_ReturnsErrorWithStatus`, `TestDoModelsFetch_BadJSON_ReturnsDecodeError`, `TestFetchOpenAICompatModels_SortsAndExtractsIDs` (also asserts the `Authorization: Bearer` header), `TestListAvailableModels_UnsupportedProviderNoBaseURL_ReturnsError`, `TestListAvailableModels_UnsupportedProviderWithBaseURL_UsesCompatFetch` — all via `httptest.Server`, no real provider network calls | 6 |
| S1-137 | Tenant LLM Provider Keys — new `LLMProviderKeysHandler` HTTP routes under `/admin/my/llm-providers/{name}/keys...` (`llm_provider_keys_test.go`, LPK-01..06): `TestLLPK_List_TenantHasNoOwnProviderRow_Returns404`, `TestLLPK_Update_InvalidKeyID_Returns400`, `TestLLPK_Create_InvalidJSON_Returns400`, `TestLLPK_Create_UnknownProvider_Returns404`, `TestLLPK_ListModels_MissingKeyID_Returns400`, `TestLLPK_Delete_UnknownProvider_Returns404` — every route resolves `{name}` to the caller's own tenant-scoped provider row first (via `GetOwnProviderRow`) before touching any key, confirmed by the 404-on-unknown-provider cases | 6 |
| S1-138 | App Canvas Debug Mode Phase 5 — trace events forwarded over the wire, not dropped (`internal/ws/handler_test.go`, `internal/sse/handler_test.go`): `TestWS_NodeTraceEventsForwarded`, `TestSSENodeTraceEventsForwarded` — `node_start`/`node_done`/`node_error` (Phase 2's Redis-stream payload shape) now hit a real case in `writeEvent`/`formatSSE` instead of falling through to `default` (WS: silently dropped; SSE: previously would have errored as "unknown event type") | 2 |
| S1-139 | App Canvas Debug Mode Phase 5 — `Lifecycle.AdmitDebug` (`internal/execution/lifecycle_test.go`): `TestLifecycle_AdmitDebug_HappyPath` (resolves EPConfig via the real epLoader, creates a real `them.runs` row via the recorder, but skips gate.Check/session.Register entirely — debug runs must never consume production admission capacity), `TestLifecycle_AdmitDebug_EPNotFound` (bad tenant/app/ep triple → AdmitErrNotFound, no run row created) | 2 |
| S1-140 | App Canvas Debug Mode Phase 5 — `AppFlowDebugService.Start` (`internal/admin/service/appflow_debug_test.go`): `TestAppFlowDebugService_Start_HappyPath` (compiles the latest draft — never `active_definition_id` — and calls `AdmitDebug` + `StartAppFlow(debug=true)`), `TestAppFlowDebugService_Start_AppNotFound`, `TestAppFlowDebugService_Start_EPSlugNotFound` (entry point missing from the application row), `TestAppFlowDebugService_Start_NoDraftSaved` (422, distinct from 404 — no draft ever saved), `TestAppFlowDebugService_Start_EPMissingFromCompiledDraft` (entry point on the app row but absent from the draft's own compiled entry points — the two can disagree after an edit) | 5 |
| S1-141 | App Canvas Debug Mode Phase 5 — `AppFlowDebugHandler.Start` HTTP mechanics (`internal/admin/appflow_debug_test.go`): `TestAppFlowDebugHandler_Start_InvalidAppID_Returns400`, `TestAppFlowDebugHandler_Start_MissingEntryPointSlug_Returns400`, `TestAppFlowDebugHandler_Start_InvalidJSON_Returns400` — pre-service-layer validation only; service behavior is covered by S1-140 | 3 |
| S1-142 | `resolveSystemAgentRole` (`internal/admin/system_agent_resolve_test.go`, `internal/admin/system_agent_resolve.go`): `TestResolveSystemAgentRole_NoRow_FallsBackToPlatform`, `TestResolveSystemAgentRole_NoRow_NoPlatformConfig_ReturnsNotOK`, `TestResolveSystemAgentRole_GeneralMode_MissingProviderName_ReturnsNotOK`, `TestResolveSystemAgentRole_GeneralMode_NoUsableKey_NeverFallsBackToPlatform` (hard-rule enforcement point — general mode with no tenant key must never substitute the platform key even when one is configured), `TestResolveSystemAgentRole_GeneralMode_UsesDefaultKeyWhenKeyIDNil`, `TestResolveSystemAgentRole_GeneralMode_UsesExplicitKeyID`, `TestResolveSystemAgentRole_CustomMode_UsesOwnFields`, `TestResolveSystemAgentRole_CustomMode_UnsetFields_FallsBackToPlatform` — **Platform-as-Tenant Phase 2 (2026-09-24, docs/PLATFORM_AS_TENANT_PLAN.md):** the "platform" fallback args these tests pass in directly are now, in production code, themselves the output of resolving the bootstrap tenant's own `them.tenant_system_agent_config` row through this same function — `resolveSystemAgentRole` itself is unchanged and these tests did not need to change. | 8 |
| S1-143 | `classifyAgent` (`internal/admin/classify_test.go`): `TestClassifyAgent_NoConfig_ReturnsEmpty`, `TestClassifyAgent_GeneralMode_NoUsableKey_ReturnsEmpty` (hard rule proven end-to-end through the real function, not just the resolver in isolation), `TestClassifyAgent_CustomMode_DispatchesToResolvedProvider` (proves the classifier no longer hardcodes Anthropic — dispatches through the shared `dispatchLLMText` the same way `synthesizeAppCard` always has), `TestClassifyAgent_InvalidCategory_FallsBackToAgent`. **Platform-as-Tenant Phase 2 (2026-09-24):** `classifyAgent` no longer reads `them.config['system_agents']` via `GetConfig` — it now resolves the "platform" fallback by calling `resolveSystemAgentRole` a second time for `tenantctx.BootstrapTenantID`, feeding that result in as the real tenant's fallback args (classifier's historical default-model quirk preserved via the new `classifierDefaultModel` constant, applied only when the bootstrap tenant's own config resolves provider+key but no model). `fakeClassifierDAL`'s now-dead `configRow`/`configErr`/`GetConfig` fields were removed (the interface no longer declares `GetConfig`); the 4 test names/behavior are otherwise unchanged. | 4 |
| S1-144 | `synthesizeAppCard` (`internal/admin/synthesize_test.go`): `TestSynthesizeAppCard_NoConfig_ReturnsNil`, `TestSynthesizeAppCard_GeneralMode_NoUsableKey_ReturnsNil` (hard rule, end-to-end), `TestSynthesizeAppCard_CustomMode_DispatchesToResolvedProvider`, `TestSynthesizeAppCard_CustomMode_UsesCustomSystemPrompt`. **Platform-as-Tenant Phase 2 (2026-09-24):** same `them.config`→bootstrap-tenant rewrite as S1-143's `classifyAgent`, no default-model quirk for this role; `fakeSynthesizerDAL`'s dead `GetConfig` plumbing removed. | 4 |
| S1-145 | Tenant LLM Provider Keys step 5 — `TenantSystemAgentConfigService` (`internal/admin/service/tenant_system_agent_config_test.go`, new `internal/admin/service/tenant_system_agent_config.go`): `TestTenantSACService_Get_UnknownRole_ReturnsValidation`, `TestTenantSACService_Get_NoRow_ReturnsCustomDefault`, `TestTenantSACService_Get_MasksCustomAPIKey`, `TestTenantSACService_Upsert_UnknownRole_ReturnsValidation`, `TestTenantSACService_Upsert_InvalidMode_ReturnsValidation`, `TestTenantSACService_Upsert_GeneralMode_MissingProviderName_ReturnsValidation`, `TestTenantSACService_Upsert_CustomMode_MissingProviderOrModel_ReturnsValidation`, `TestTenantSACService_Upsert_CustomMode_FirstTimeNoKey_ReturnsValidation`, `TestTenantSACService_Upsert_GeneralMode_PersistsProviderAndKeyID`, `TestTenantSACService_Upsert_CustomMode_EncryptsKeyBeforePersist`, `TestTenantSACService_Upsert_CustomMode_NoNewKey_KeepsExistingEncrypted` | 11 |
...
| S1-150 | Security Scanner as a third `tenant_system_agent_config` role (`internal/admin/security_scan_llm_test.go`, `internal/admin/security_scan_llm.go`) — moves the LLM card/skill risk analysis out of the Python `them-security-agent` container (which used one hardcoded platform-wide Anthropic key for every tenant) into go-bridge, resolved per-tenant the same way `classifyAgent`/`synthesizeAppCard` already are: `TestLLMCardAnalysis_NoConfig_ReturnsDegraded`, `TestLLMCardAnalysis_GeneralMode_NoUsableKey_ReturnsDegraded` (hard rule, end-to-end — the bootstrap tenant's key configured must never be substituted), `TestLLMCardAnalysis_CustomMode_DispatchesToResolvedProvider`, `TestMergeSecurityScanResult_Degraded_AddsAnalysisFindingAndPenalty`, `TestMergeSecurityScanResult_HighRiskFinding_AppliesFullPenalty`, `TestMergeSecurityScanResult_PenaltyCappedAt40`, `TestMergeSecurityScanResult_UsesLLMSummaryWhenPresent` — `mergeSecurityScanResult` ports the exact score/risk/findings-merge arithmetic that used to live in `scanner.py`'s `run_scan`, now applied in Go after the Python scanner (probes-only now) returns. Closes a pre-existing gap: `runScanJob`/`SecurityScan` had zero direct tests before this change (out of scope to add full coverage for the whole background-job flow here; only the two new pure-logic pieces got tests). **Platform-as-Tenant Phase 2 (2026-09-24):** same `them.config`→bootstrap-tenant rewrite as S1-143/144; `fakeSecurityScanDAL`'s dead `GetConfig`/`securityScannerPlatformConfigRow` helper removed. | 7 |
| **removed (Platform-as-Tenant Phase 2, 2026-09-24)** | `docs/PLATFORM_AS_TENANT_PLAN.md` Phase 2 deleted the now-redundant NULL-tenant ("platform-only") Go code paths once Phase 1 migrated their data to the bootstrap tenant. Removed in this pass, with no replacement test (the deleted production code no longer exists): former S1-151 `llm_provider_keys_platform_test.go` (`LLMProviderKeysHandler` platform routes, 6 tests) — file `internal/admin/llm_provider_keys_platform.go` deleted, its `PlatformRoutes` mount removed from `router.go`; former S1-152 `resolvePlatformSystemAgentRole` tests (6 tests) — function and its `platformSystemAgentRoleResolverDAL` interface deleted from `system_agent_resolve.go`; former S1-153 `GetPlatformProviderRow` tests (2 tests) — function deleted from `service/llm_providers.go`, its DAL twin `GetProviderByNamePlatform` deleted from `dal/llm_providers.go` (the one legitimate caller, `UpsertForTenant`'s platform-template lookup, now calls `GetProviderByNameForTenant(ctx, name, tenantctx.BootstrapTenantID)` instead — covered by existing `TestTenantProvider_Upsert_*` tests in `service/llm_providers_test.go`, updated in place); former S1-154 `system_agents_integration_test.go` (`SystemAgentsHandler` platform-global `/admin/system-agents`, 2 tests) — file `internal/admin/system_agents.go` deleted entirely (its `Routes`/`NewSystemAgentsHandler` mount removed from `router.go`); `internal/admin/system_agent_resolve_test.go`'s platform-specific test block (was part of old S1-152, listed there) removed along with the fake's `platformProvider`/`platformProviderErr` fields (now unused). Net: **-16 tests removed**, 0 replacement tests needed since the deleted code is gone, not relocated. | -16 (net removal) |
| S2-17 | Platform-as-Tenant Phase 2 (2026-09-24, docs/PLATFORM_AS_TENANT_PLAN.md) — `ListProvidersForTenant` and `GetProviderBaseURLs` rewritten to reference the bootstrap tenant's real `tenant_id` explicitly instead of a `tenant_id IS NULL` check that stopped matching anything after Phase 1's migration (`dal/llm_providers.go`, `dal/app_config.go`, new `internal/admin/dal/platform_as_tenant_phase2_integration_test.go`, first direct integration coverage either query has ever had): `TestDAL_ListProvidersForTenant_FallsBackToBootstrapTenantDefault`, `TestDAL_ListProvidersForTenant_OwnRowWinsOverBootstrapDefault`, `TestDAL_ListProvidersForTenant_BootstrapTenantItself_NoDuplicate` (querying as the bootstrap tenant itself must not double-count its own row), `TestDAL_GetProviderBaseURLs_FallsBackToBootstrapTenantDefault`, `TestDAL_GetProviderBaseURLs_OwnRowWinsOverBootstrapDefault`. Run live against this box's already-migrated Postgres (same DB Phase 1 migrated), all 5 passed. | 5 |
| S1-155 | AppFlow Runtime Params (docs/APPFLOW_RUNTIME_PARAMS_PLAN.md) — `llm` node kind declares a required `llm_credential` runtime param (`internal/appflow/noderegistry_test.go`): `TestAllAppCanvasNodeInfos_LLMDeclaresCredentialRuntimeParam` (also asserts no other of the 6 kinds unexpectedly gained one) | 1 |
| S1-156 | AppFlow Runtime Params — `AppFlowDebugService.Start` per-node credential validation/resolution (`internal/admin/service/appflow_debug_test.go`, new `appflow_debug_credentials.go`): `TestAppFlowDebugService_Start_MissingRequiredOverride_FailsBeforeAdmit` (a required param with no override entry fails validation before `AdmitDebug` is ever called — a validation failure must never consume a debug run slot), `TestAppFlowDebugService_Start_CustomMode_UsesLiteralKey_PerNode` (two independent LLM nodes each get their own `Set` call with their own key — never merged/deduped, the core "per node, not global" requirement), `TestAppFlowDebugService_Start_GeneralMode_NoUsableKey_FailsClearly`, `TestAppFlowDebugService_Start_CredentialStoreWriteFails_SurfacesError` | 4 |
| S1-157 | AppFlow Runtime Params — `/ws/dashboard` `run:*` tenant ownership gate (`internal/dashboard/handler_test.go`, new `run_ownership.go`) — closes a real cross-tenant IDOR found this session: `IsValidChannel` only validated channel-name shape, never whether the caller's tenant actually owned the run, so any authenticated caller of any tenant could subscribe to any other tenant's `run:{id}` and read its live trace. `TestDashboard_RunChannel_WrongTenantRefused`, `TestDashboard_RunChannel_OwnershipCheckErrorRefusesNotTails` (fails closed, not open), `TestDashboard_RunChannel_NilOwnerRefusesEntirely` (nil must refuse, not skip the check) | 3 |
| S1-158 | AppFlow Runtime Params — `dbLLMCaller.Complete`'s debug-override branch (`cmd/dag-worker/dbllmcaller_debug_test.go`, new file — `dbLLMCaller` had zero direct tests before this change): `TestDBLLMCaller_Complete_Debug_UsesOverride`, `TestDBLLMCaller_Complete_Debug_MissingOverride_FailsClearly` (absence-means-missing for a debug run — must fail loudly, never silently fall through to normal tenant/platform key resolution), `TestDBLLMCaller_Complete_Debug_StoreErrorFailsClearly`, `TestDBLLMCaller_Complete_Debug_NilStore_FailsClearly` (a real bug caught by this test before it shipped: a nil `debugStore` on a `Debug:true` call was falling through to normal resolution and nil-panicking on the unconfigured resolver instead of failing via the explicit error path — fixed in the same commit), `TestDBLLMCaller_Complete_Debug_DifferentNode_DoesNotLeakOverride` | 5 |
| S1-159 | AppFlow Runtime Params review follow-up (d941aca3) issue #2/#3 — `resolveLLMOverride`'s General-mode key/provider mismatch check and per-node model selection (`internal/admin/service/appflow_debug_credentials_test.go`, new file): `TestResolveLLMOverride_General_KeyBelongsToDifferentProvider_Rejected` (closes a real bug: `GetLLMProviderKey` only scopes by id+tenant, never `llm_provider_id`, so a key_id belonging to a different provider was previously silently accepted), `TestResolveLLMOverride_General_KeyBelongsToSameProvider_Succeeds`, `TestResolveLLMOverride_General_DefaultKey_NoMismatchCheckNeeded`, `TestResolveLLMOverride_General_Model_NotInAllowedList_Rejected`, `TestResolveLLMOverride_General_Model_InAllowedList_Succeeds`, `TestResolveLLMOverride_General_Model_Empty_UsesProviderDefault`, `TestResolveLLMOverride_General_NoAllowedModelsConfigured_AnyModelAccepted`, `TestResolveLLMOverride_Custom_BaseURLPreserved`, `TestResolveLLMOverride_General_BaseURLFromProviderRow_Preserved` | 9 |
| S1-160 | AppFlow debug panel presets (db/111) — per-(tenant,user,application) saved presets so a user can reload their last debug-panel entry point/message/step-mode/LLM overrides instead of re-entering them; personal, never shared (`internal/admin/service/appflow_debug_presets_test.go`, new `internal/admin/service/appflow_debug_presets.go` + `internal/admin/dal/appflow_debug_presets.go` + `internal/admin/appflow_debug_presets.go`): `TestAppFlowDebugPresetService_Save_MissingName_ReturnsValidation`, `TestAppFlowDebugPresetService_Save_MissingEntryPointSlug_ReturnsValidation`, `TestAppFlowDebugPresetService_Save_EncryptsAPIKeyBeforePersist` (a Custom-mode `api_key` must be Fernet-encrypted before it ever reaches the DAL — the plaintext value must never appear in the stored JSONB), `TestAppFlowDebugPresetService_Save_NoAPIKey_StoresNoEncryptedField`, `TestAppFlowDebugPresetService_List_MasksStoredAPIKey` (round-trips a real encrypted value through List and asserts the plaintext never comes back), `TestAppFlowDebugPresetService_Delete_NoRows_ReturnsNotFound`, `TestAppFlowDebugPresetService_Delete_Success` | 7 |
| S1-160 | AppFlow Runtime Params review follow-up (d941aca3) issue #1 — per-node BaseURL actually reaches the provider (`cmd/dag-worker/dbllmcaller_debug_test.go`, extends S1-158): `TestDBLLMCaller_Complete_Debug_UsesOverrideBaseURL` — two nodes, two fake HTTP servers, proves each node's request lands on ITS OWN configured endpoint (closes a real bug: `BaseURL` was stored in the override but `Complete` never passed it into provider creation, so every debug node silently used the worker's static per-provider base-URL map or the provider's hardcoded default instead) | 1 |
| S1-161 | AppFlow Runtime Params review follow-up (d941aca3) issue #4 — `FinalizeRunActivity`'s debug credential cleanup call (`internal/appflow/workflow_test.go`, extends AF-WF-01/02): `TestFinalizeRunActivity_Debug_CleansUpCredentials` (fires with correct tenant/run scoping on a debug run's terminal state), `TestFinalizeRunActivity_NotDebug_NeverCleansUp` (never fires for a production run — no wasted Redis SCAN), `TestFinalizeRunActivity_Debug_NilCleaner_NoOp`, `TestFinalizeRunActivity_Debug_CleanupErrorDoesNotFailActivity` (a cleanup failure must not fail the otherwise-idempotent finalize activity — an orphaned entry still expires via its own TTL) | 4 |
| S1-162 | AppFlow Runtime Params review follow-up (c1f01aa2) — bounded debug-run lifetime enforcement. `Lifecycle.StartAppFlow`'s `WorkflowRunTimeout` wiring (`internal/execution/lifecycle_test.go`, extends S1-129/134/139): `TestLifecycle_StartAppFlow_Debug_EnforcesMaxLifetimeTimeout` (debug=true → `WorkflowRunTimeout` = `appflow.DebugRunMaxLifetime`, closes the gap where debug runs previously had NO workflow-level wall-clock bound at all), `TestLifecycle_StartAppFlow_NotDebug_NoWorkflowRunTimeoutByDefault` (non-debug, no `TemporalCfg` → zero value, unchanged prior behavior), `TestLifecycle_StartAppFlow_NotDebug_UsesConfiguredTimeoutNotDebugConstant` (non-debug with a configured `WorkflowTimeoutS` uses that value, never the debug constant). `debugcred.TTL` derivation (`internal/debugcred/ttl_test.go`, new file): `TestTTL_DerivedFromDebugRunMaxLifetimePlusCleanupMargin` (proves `TTL == appflow.DebugRunMaxLifetime + CleanupMargin == 3h40m` — replaces the prior round's independently-guessed fixed 30-minute TTL, so credential retention can never disagree with the run's own enforced timeout). `DebugStartResult.ExpiresAt` computation (`internal/admin/service/appflow_debug_test.go`, extends S1-140): `TestAppFlowDebugService_Start_ExpiresAt_IsStartedAtPlusDebugRunMaxLifetime` (bounded before/after timestamp check that `ExpiresAt` = call-time + `DebugRunMaxLifetime`, surfaced to the debug panel so the bound is visible up front, not just discovered via a later timeout error) | 5 |
| S1-163 | App Canvas Debug Mode Phase 6 — Step controls, lockstep multi-branch pausing (docs/APP_CANVAS_DEBUG_PLAN.md). Backend mechanism: `AppFlowSignalStep` signal + a shared `stepTick.Gen` counter + `workflow.Await` (NOT one signal per paused branch — a Temporal named signal channel is FIFO, one `SignalWorkflow` call wakes exactly one blocked `Receive`, never all of them; verified against the Go SDK's vendored source before implementing). `internal/appflow/workflow_temporal_test.go` (extends `AppFlowTraceWorkflowTestSuite`, AF-STEP-01..03): `TestStepMode_NoSignalSent_WorkflowNeverCompletes` (genuinely blocked at the first node, not running through), `TestStepMode_TwoSignals_AdvancesOneNodeAtATime` (sequential chain, one node per signal, in order), `TestStepMode_ForkedBranches_OneSignalReleasesBothInLockstep` (the core guarantee: ONE signal, sent once both fork branches are independently paused at their own first tick, releases BOTH simultaneously — proven against a real Temporal `TestWorkflowEnvironment`, not mocked). `internal/admin/service/appflow_debug_test.go`: `TestAppFlowDebugService_Start_StepModeTrue_PropagatesToWorkflowInput`. New route `POST /admin/applications/{id}/debug/{run_id}/step` (`internal/admin/appflow_debug_test.go`, AFD-4..7): `TestAppFlowDebugHandler_Step_InvalidRunID_Returns400`, `TestAppFlowDebugHandler_Step_RunNotFound_Returns404` (tenant-scoped `GetRun` lookup — same cross-tenant-IDOR-safe pattern as `HILApprovalsHandler`, never trusts `{run_id}` alone), `TestAppFlowDebugHandler_Step_NoTemporalConfigured_Returns503`, `TestAppFlowDebugHandler_Step_Success_SignalsWorkflow`. New `node_paused` trace event added to `ws/handler.go` and `sse/handler.go`'s existing `node_start`/`node_done`/`node_error` forwarding case (no new test needed — same generic `ev.Type` passthrough already covered by S1-52/S1-149's tests). | 8 |
| S1-164 | App Canvas Debug Mode Phase 6 review fix — `mainLastSeenGen` staleness across a fork (`internal/appflow/workflow_temporal_test.go`, AF-STEP-04): a real bug found in review, not caught by S1-163's own lockstep test (which had no node after the join to observe it with). After `wg.Wait(ctx)` returns from a fork's branches, `tick.Gen` has advanced from whatever signals released the branches, but the main loop's own step cursor (`mainLastSeenGen`) was never updated — so the very next `stepGate` call (for the node after the join) saw `tick.Gen` already ahead of its stale cursor and ran immediately, for free, without its own Step click. Fixed by syncing `mainLastSeenGen = tick.Gen` (guarded `tick != nil`) right after `wg.Wait(ctx)` in the fork case (`workflow.go`). `TestStepMode_NodeAfterJoin_RequiresItsOwnSeparateStepClick` reproduces it with fork→2 branches→join→condPost→end and asserts `condPost` does not complete before its own, separate, 3rd signal — confirmed failing before the fix (workflow completed on 3 signals instead of 4, `condPost` ran unreleased) and passing after. | 1 |
| S1-165 | Platform-as-Tenant Phase 6 — live agent resolution for debug, closing the "must publish first" gap `docs/APP_CANVAS_DEBUG_PLAN.md` flagged as a known limitation (`internal/admin/service/appflow_debug_agent_resolve_test.go`, new file; `AppFlowDebugService.resolveDraftAgentIDs` in `appflow_debug.go`): `TestResolveDraftAgentIDs_NilRegistry_NoOp` (no registry wired → exact prior behavior, unresolved_agent still fires), `TestResolveDraftAgentIDs_UnpublishedDraft_ResolvesLive` (the core fix: an unpublished draft's agent node resolves via the same `RegistryResolver.ResolveForPublish` `PublishDefinition` uses), `TestResolveDraftAgentIDs_AlreadyStamped_NeverOverwritten` (a real publish-time stamp always wins, never re-resolved), `TestResolveDraftAgentIDs_RegistryResolveFails_LeavesUnresolved`, `TestResolveDraftAgentIDs_AgentRowMissing_LeavesUnresolved` (resolved component_definitions row but no matching `agents` row — e.g. deleted — must not count as resolved), `TestResolveDraftAgentIDs_AgentExistsCheckErrors_PropagatesError` (a real DB error must propagate, not be swallowed like not-found), `TestResolveDraftAgentIDs_NonAgentComponents_Skipped`, and two full `Start`-level end-to-end tests: `TestAppFlowDebugService_Start_UnpublishedAgentDraft_NoLongerBlocked` (the exact previously-broken scenario now succeeds) and `TestAppFlowDebugService_Start_UnpublishedAgentDraft_NilRegistry_StillBlocked` (fallback behavior unchanged when no registry is wired). Found live during this phase's own UI walkthrough — see `docs/PLATFORM_AS_TENANT_PLAN.md`'s Phase 6 section for the full live-verification trace (a real, still-unpublished app's debug/start call went from `unresolved_agent` to a real 200 run_id). | 9 |
| S1-166 | App Canvas Debug Mode — Temporal UI deep-link, `DebugStartResult.WorkflowID`/`debugStartResponse.WorkflowID` (`internal/admin/service/appflow_debug_test.go`, extends S1-140/AF-TR): `TestAppFlowDebugService_Start_HappyPath` strengthened to assert `result.WorkflowID == "appflow:tenant-1:run-1"` (`appflow.WorkflowIDForRun(handle.EPConfig.TenantID, ...)`, matching the exact ID the workflow itself is started with). Also fixed a latent nil-pointer gap this change exposed: 6 existing `execution.ExecutionHandle{RunID: "run-1"}` test fixtures across `appflow_debug_test.go`/`appflow_debug_agent_resolve_test.go` had never set `EPConfig`, which only 2 of the file's pre-existing code paths (LLM General-mode credential writes) ever dereferenced — this new WorkflowID line dereferences it unconditionally at the end of `Start`, turning the gap into an immediate panic across most of the file. Fixed by giving every fixture a real `EPConfig` (matching what `AdmitDebug` always returns after a successful load), not a defensive nil-check for a state that can't occur in production. | 0 net new (1 existing test strengthened; 8 fixtures corrected) |
| S1-167 | App Canvas Debug Mode — "smart debug log" structured result summary, `AppFlowDebugService.GetResult` (`internal/admin/service/appflow_debug_result_test.go`, new file): `TestGetResult_AllNodesCompleted_OkTrue` (happy path — Ok=true, full ordered node list with output/latency), `TestGetResult_NodeFailed_OkFalse_ReportsFirstFailure` (the core case: a real failing node's id+error surfaced at the top level, not just buried in the per-node list), `TestGetResult_MultipleFailures_ReportsOnlyFirst` (two independent failed nodes → reports the first one in execution order, not the last or an arbitrary one), `TestGetResult_NoStepsYet_OkTrue` (zero steps is vacuously "nothing failed," not a fabricated failure), `TestGetResult_RunNotFound_ReturnsErrNotFound`. New `AppFlowDebugHandler.Result` route, `GET /applications/{id}/debug/{run_id}/result` (`internal/admin/appflow_debug_test.go`, AFD-8/9): `TestAppFlowDebugHandler_Result_InvalidRunID_Returns400`, `TestAppFlowDebugHandler_Result_RunNotFound_Returns404` (same `fakeDB{queryRowErr: pgx.ErrNoRows}` pattern as the Step route's own not-found test, since `GetRunDetail`'s first internal call is the same single-row `GetRun` lookup). `AppFlowDebugDAL` widened with `GetRunDetail` — `dal.DB` already implements it, only the interface subset and `fakeAppFlowDebugDAL` needed a new method. Verified live against the real, rebuilt `them-go-bridge`: a successful run's result showed `ok:true` with real LLM output per node; a run started with a deliberately invalid API key showed `ok:false` with the exact real Anthropic 401 error message at both the top level and the specific failed node. | 7 |
| **S1 total** | | **1496** (1484 + 1 AF-WF-17 + 7 S1-169 A2A response-kind decoding + 3 AF-WF-18/19/20 + 1 S1-170 File Guard node_id scoping fix, 2026-09-26) |
...
| **flaky (pre-existing)** | `internal/appflow.TestAppFlowTraceWorkflowTestSuite/TestForkJoin_EmitsTraceForAllNodes` (`workflow_temporal_test.go`, AF-TR-W03) fails intermittently under `go test -race` (reproduced 1-of-3 runs in isolation with `-count=3`) — race detected inside the Temporal SDK's own `testWorkflowEnvironmentImpl.ExecuteActivity` (`vendor/go.temporal.io/sdk/internal/internal_workflow_testsuite.go:1181`/`1202`), triggered by the test's concurrent fork-branch goroutines each calling `workflow.ExecuteActivity` via `traceNode`/`walkBranch`. Confirmed pre-existing and unrelated to the App Canvas Debug Mode runtime-params session (2026-09-23) that found it while running `go test -race` on touched packages: that session's only edits to `workflow.go`/`graph.go` were one non-concurrent struct-literal field addition each (`Debug: input.Debug`) in the unrelated LLM-node dispatch path, nowhere near the Fork/Join/Condition trace concurrency this race is in. Not fixed here — Temporal SDK test-harness internals, out of scope; flagged for a dedicated fix. | not counted above |
| **flaky (pre-existing)** | `internal/a2a` package hangs and times out (600s) under a full `go test ./...` run (goroutine stack dump inside vendored `a2aproject/a2a-go`'s `taskexec`/`eventpipe` internals — a `select`/`Read` blocked 9+ minutes). Confirmed pre-existing and unrelated to the Platform-as-Tenant Phase 6 session (2026-09-24) that found it: `go test ./internal/a2a/...` in isolation passes cleanly (`ok`, 0.18s) both before and after that session's only change (`internal/db/platform_as_tenant_rls_integration_test.go`, a cleanup-ordering fix touching no `a2a` code). Reproduces only under the full-suite run's load/timing, not in isolation — not fixed here, flagged for a dedicated fix. | not counted above |
...
| S2-13 | Tenant LLM Provider Keys step 5 — `them.tenant_system_agent_config` DAL against live Postgres (`internal/admin/dal/tenant_system_agent_config_integration_test.go`, migration `db/106_tenant_system_agent_config.sql`, applied live this session): `TestDAL_TenantSystemAgentConfig_GetNoRow`, `TestDAL_TenantSystemAgentConfig_UpsertGeneralMode`, `TestDAL_TenantSystemAgentConfig_UpsertCustomMode`, `TestDAL_TenantSystemAgentConfig_UpsertReplacesExisting`, `TestDAL_TenantSystemAgentConfig_TenantIsolation`, `TestDAL_TenantSystemAgentConfig_KeyDeletedSetsKeyIDNull` (proves the `ON DELETE SET NULL` FK to `llm_provider_keys` behaves as designed, not just as declared) | 6 |
| **removed (Platform-as-Tenant Phase 2, 2026-09-24)** | Former S2-14 — 4 `llm_provider_keys` "platform-owned" (`tenant_id IS NULL`) integration tests (`TestDAL_ProviderKey_PlatformOwned_CreateAndList`, `_InvisibleToTenantQueries`, `_NameUniqueAmongPlatformKeysOnly`, `_SetDefault_ClearsPrevious`) deleted from `internal/admin/dal/llm_provider_keys_integration_test.go`. `CreateLLMProviderKey(TenantID: nil)` is unreachable from any HTTP route since Phase 2 deleted `llm_provider_keys_platform.go` (ex-S1-151) — the only caller that ever passed `nil`. Discovered because `_NameUniqueAmongPlatformKeysOnly` started failing after Phase 1's migration replaced the NULL-partial unique index with a plain composite `(llm_provider_id, tenant_id, name)` index — Postgres never treats two NULLs as equal in a unique index, so duplicate platform-owned key names silently stopped colliding. Since the code path is dead, not live, the fix is deleting the test rather than restoring a partial index nobody calls into anymore. Net: **-4 tests removed**, no replacement needed. | -4 (net removal) |
| S2-15 | AppFlow Runtime Params — `internal/debugcred.Store` against live Redis (`internal/debugcred/store_integration_test.go`, new package): `TestIntegration_Store_SetGet_RoundTrips`, `TestIntegration_Store_GetSurvivesRepeatedReads` (proves the value is NOT consumed on read — a Temporal activity retry re-runs the whole activity including the credential lookup, so a second `Get` for the same key must still find it), `TestIntegration_Store_Get_NotFound_ReturnsFalseNotError`, `TestIntegration_Store_TenantScoping_DifferentTenantsDoNotCollide` (two tenants with the same run_id/node_id — pathological, since run_id is already a UUIDv4 — still can't see each other's override), `TestIntegration_Store_Delete_RemovesEntry`. Run against the live `them-redis` container this session (`docker run --network them-network -e REDIS_ADDR=them-redis:6379 ... go test -tags=integration ./internal/debugcred/...`), not just started fresh — all 5 passed. | 5 |
| S2-16 | AppFlow Runtime Params review follow-up (d941aca3) issue #4 — `Store.DeleteAllForRun` against live Redis (`internal/debugcred/store_integration_test.go`, extends S2-15): `TestIntegration_Store_DeleteAllForRun_RemovesEveryNode` (3 nodes, one call removes all 3), `TestIntegration_Store_DeleteAllForRun_DoesNotTouchOtherRuns` (the pattern-scoped SCAN doesn't over-delete — a different run's entry survives), `TestIntegration_Store_DeleteAllForRun_NoEntries_NoError`. Run against live `them-redis` this session, all 3 passed. | 3 |
| S2-18 | Platform-as-Tenant Phase 3 (2026-09-24, docs/PLATFORM_AS_TENANT_PLAN.md) — dedicated RLS-enforced regression tests for decision 7's asymmetric visibility, run via `them_app`/`BeginTenantTx` (real RLS enforcement) against this box's live, already-migrated Postgres — not the BYPASSRLS admin pool Phase 2's own tests used (new `internal/db/platform_as_tenant_rls_integration_test.go`): `TestRLS_BootstrapTenant_LLMProviders_VisibleCrossTenant_ReadOnly` (another tenant's `TenantTx` SELECTs the bootstrap tenant's `llm_providers` row fine — `llm_providers_read`'s explicit `OR tenant_id = <bootstrap-uuid>` — but its UPDATE/DELETE attempts affect 0 rows / error, confirmed unchanged via the Admin pool afterward), `TestRLS_BootstrapTenant_LLMProviders_AppRoleHasNoWriteGrant` (real finding, not a regression: `them_app` has only `SELECT` GRANTed on `them.llm_providers` — `llm_providers_write`'s RLS policy is unreachable via `them_app` for ANY tenant including the bootstrap tenant itself; the real write path is `LLMProviderService.UpsertTenantProvider`/`CreateProvider` via the Admin/BYPASSRLS pool, confirmed still working post-db/110), `TestRLS_BootstrapTenant_LLMProviderKeys_InvisibleCrossTenant` (the asymmetric half of decision 7: another tenant's `TenantTx` sees **0 rows** of the bootstrap tenant's `llm_provider_keys` — no bootstrap-visibility clause exists on `llm_provider_keys_tenant_isolation`, unlike `llm_providers_read`; also confirms `them_app` has no write GRANT on this table either, same pattern), `TestRLS_BootstrapTenant_LLMProviderKeys_OwnTenantSeesOwnKey` (control case — the bootstrap tenant acting as itself still sees its own key normally). All 4 run live against this box's `them-postgres` via `them_app`/`them_admin` pools (`docker run --network them-network ... go test -tags=integration -run TestRLS_BootstrapTenant ./internal/db/...`), all passed. Turns Phase 1's one-time manual `SET ROLE` verification into a permanent regression test, per the plan's Phase 3 description. | 4 |
| S1-168 | AppFlow Named Data Ports Phase 1 (docs/APPFLOW_NAMED_PORTS_PLAN.md) — backend registry metadata only, zero runtime behavior change. `llm` node kind gains `OutputPorts: [{ID: "output", Label: "Output", TypeHint: "text"}]` in `internal/appflow/noderegistry.go`, aliasing its existing `output_var` config field as a real, drag-connectable data port (`condition` gets none — its two outputs are control branches, not a data value; confirmed no other of the 6 kinds touches `FlowVars` at runtime before excluding them, per the plan's scope-narrowing decision). `internal/appflow/noderegistry_test.go`: `TestAllAppCanvasNodeInfos_LLMHasOutputPort` (asserts exactly 1 output port on `llm`, ID `output`, non-empty label, `AcceptsDynamicInputs=true`; asserts zero `OutputPorts` on all other 5 kinds). | 1 |
| S1-170 | AppFlow A2A Response Kinds Phase 2 (docs/APPFLOW_A2A_RESPONSE_KINDS_PLAN.md) — File Guard node_id scoping fix, `internal/middleware/gate_test.go` (extends S1-84/S1-87): `TestFileGate_NodeIDScoping` proves the real bug found before writing this phase — `loadWiringCfg` previously matched by `agent.slug` only, so two canvas instances of the same agent would share (or collide on) one `middleware_wirings` row even though the table already had a real `node_id` column with its own unique index (`uq_mw_wiring_app_node`) specifically to support independent per-instance wirings. Test drives two `InterceptInline` calls with the same `AgentSlug` but different `NodeID` against a fake DB keyed on the `node_id` query arg — one resolves enabled, the other disabled, proving they no longer leak into each other. | 1 |
| **S2 total** | | **86** (82 + 4 [S2-18], Platform-as-Tenant Phase 3, 2026-09-24) |
| S3 live | manual | 23 |
| S1-IDP | idpcrypto (AES-256-GCM encrypt/decrypt for IdP client_secret): IDP-1..9 | 9 |
| **`go test ./...` total** | | **1326** (S1 total; this line is out of sync with S1/S2 subtotals above pre-dating this entry — full reconciliation not in scope for this change) |
