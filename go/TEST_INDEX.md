...
| `internal/admin/llm_provider_keys_platform.go` (`LLMProviderKeysHandler` platform routes), `internal/admin/dal/llm_provider_keys.go` (`tenant_id *string` widened, db/108), `db/108_platform_llm_provider_keys.sql` | S1-151 + S2-14 (integration) |
| `internal/admin/system_agent_resolve.go` (`resolvePlatformSystemAgentRole`, `platformSystemAgentRoleResolverDAL`) | S1-152 |
| `internal/admin/service/llm_providers.go` (`GetPlatformProviderRow`) | S1-153 |
| `internal/admin/system_agents.go` (`saRoleStored.Mode`/`KeyID`, `Put` mode handling) | S1-154 (integration) |
...
| S1-142 | Tenant LLM Provider Keys step 5 — `resolveSystemAgentRole` (`internal/admin/system_agent_resolve_test.go`, new `internal/admin/system_agent_resolve.go`): `TestResolveSystemAgentRole_NoRow_FallsBackToPlatform`, `TestResolveSystemAgentRole_NoRow_NoPlatformConfig_ReturnsNotOK`, `TestResolveSystemAgentRole_GeneralMode_MissingProviderName_ReturnsNotOK`, `TestResolveSystemAgentRole_GeneralMode_NoUsableKey_NeverFallsBackToPlatform` (hard-rule enforcement point — general mode with no tenant key must never substitute the platform key even when one is configured), `TestResolveSystemAgentRole_GeneralMode_UsesDefaultKeyWhenKeyIDNil`, `TestResolveSystemAgentRole_GeneralMode_UsesExplicitKeyID`, `TestResolveSystemAgentRole_CustomMode_UsesOwnFields`, `TestResolveSystemAgentRole_CustomMode_UnsetFields_FallsBackToPlatform` | 8 |
| S1-143 | Tenant LLM Provider Keys step 5 — `classifyAgent` now tenant-aware and multi-provider (`internal/admin/classify_test.go`): `TestClassifyAgent_NoConfig_ReturnsEmpty`, `TestClassifyAgent_GeneralMode_NoUsableKey_ReturnsEmpty` (hard rule proven end-to-end through the real function, not just the resolver in isolation), `TestClassifyAgent_CustomMode_DispatchesToResolvedProvider` (proves the classifier no longer hardcodes Anthropic — dispatches through the shared `dispatchLLMText` the same way `synthesizeAppCard` always has), `TestClassifyAgent_InvalidCategory_FallsBackToAgent`. Closes a pre-existing gap: `classifyAgent` had zero direct tests before this change. | 4 |
| S1-144 | Tenant LLM Provider Keys step 5 — `synthesizeAppCard` now tenant-aware (`internal/admin/synthesize_test.go`): `TestSynthesizeAppCard_NoConfig_ReturnsNil`, `TestSynthesizeAppCard_GeneralMode_NoUsableKey_ReturnsNil` (hard rule, end-to-end), `TestSynthesizeAppCard_CustomMode_DispatchesToResolvedProvider`, `TestSynthesizeAppCard_CustomMode_UsesCustomSystemPrompt`. Closes a pre-existing gap: `synthesizeAppCard` had zero direct tests before this change. | 4 |
...
| S1-150 | Security Scanner as a third `tenant_system_agent_config` role (`internal/admin/security_scan_llm_test.go`, new `internal/admin/security_scan_llm.go`) — moves the LLM card/skill risk analysis out of the Python `them-security-agent` container (which used one hardcoded platform-wide Anthropic key for every tenant) into go-bridge, resolved per-tenant the same way `classifyAgent`/`synthesizeAppCard` already are: `TestLLMCardAnalysis_NoConfig_ReturnsDegraded`, `TestLLMCardAnalysis_GeneralMode_NoUsableKey_ReturnsDegraded` (hard rule, end-to-end — a platform key configured must never be substituted), `TestLLMCardAnalysis_CustomMode_DispatchesToResolvedProvider`, `TestMergeSecurityScanResult_Degraded_AddsAnalysisFindingAndPenalty`, `TestMergeSecurityScanResult_HighRiskFinding_AppliesFullPenalty`, `TestMergeSecurityScanResult_PenaltyCappedAt40`, `TestMergeSecurityScanResult_UsesLLMSummaryWhenPresent` — `mergeSecurityScanResult` ports the exact score/risk/findings-merge arithmetic that used to live in `scanner.py`'s `run_scan`, now applied in Go after the Python scanner (probes-only now) returns. Closes a pre-existing gap: `runScanJob`/`SecurityScan` had zero direct tests before this change (out of scope to add full coverage for the whole background-job flow here; only the two new pure-logic pieces got tests). | 7 |
| S1-151 | The-M admin (platform) gets the same named-keys system tenants have (db/108 — `llm_provider_keys.tenant_id` made nullable, NULL = platform-owned): new `LLMProviderKeysHandler` platform-scoped routes under `/admin/llm-providers/{name}/keys...` (`internal/admin/llm_provider_keys_platform_test.go`, LPKP-01..06): `TestLLPKPlatform_List_UnknownProvider_Returns404`, `TestLLPKPlatform_Update_InvalidKeyID_Returns400`, `TestLLPKPlatform_Create_InvalidJSON_Returns400`, `TestLLPKPlatform_Create_UnknownProvider_Returns404`, `TestLLPKPlatform_ListModels_MissingKeyID_Returns400`, `TestLLPKPlatform_Delete_UnknownProvider_Returns404` | 6 |
| S1-152 | `resolvePlatformSystemAgentRole` (`internal/admin/system_agent_resolve_test.go`) — the-M admin's own general/custom mode resolution for classifier/card_synthesizer/security_scanner, mirroring `resolveSystemAgentRole`'s tenant logic against platform-owned (`tenant_id IS NULL`) provider/key rows instead: `TestResolvePlatformSystemAgentRole_Disabled_ReturnsNotOK`, `TestResolvePlatformSystemAgentRole_CustomMode_UsesOwnFields`, `TestResolvePlatformSystemAgentRole_ModeEmpty_TreatedAsCustom` (roles saved before this feature existed have no mode field and must still work), `TestResolvePlatformSystemAgentRole_GeneralMode_MissingProvider_ReturnsNotOK`, `TestResolvePlatformSystemAgentRole_GeneralMode_UsesDefaultKeyWhenKeyIDNil`, `TestResolvePlatformSystemAgentRole_GeneralMode_NoUsableKey_ReturnsNotOK` | 6 |
| S1-153 | `LLMProviderService.GetPlatformProviderRow` (`internal/admin/service/llm_providers_test.go`): `TestGetPlatformProviderRow_NotFound_ReturnsErrNotFound`, `TestGetPlatformProviderRow_Found_ReturnsPlatformRow`. Fixed a pre-existing test-fake bug found in passing: `fakeDal.GetProviderByNamePlatform` incorrectly checked `tenantProviderNotFound` instead of its own flag — added `platformProviderNotFound` and fixed the one existing test (`TestTenantProvider_Upsert_PlatformNotFound_ReturnsNotFound`) that had been relying on the shared flag by coincidence. | 2 |
| S1-154 | `SystemAgentsHandler` (platform-global `/admin/system-agents`) integration tests against live Postgres, its first direct tests ever (`internal/admin/system_agents_integration_test.go`): `TestSystemAgents_Put_GeneralMode_RoundTripsModeAndKeyID` (mode/key_id survive a real JSONB round trip through `them.config`), `TestSystemAgents_Put_CustomMode_EncryptsKeyAndMasksOnRead` (existing encrypt-on-write/mask-on-read behavior unaffected by adding mode/key_id). Also fixed a pre-existing bit-rotted sibling file in the same package, `tokens_sessions_integration_test.go`, which failed to even *compile* under `-tags=integration` (`dal.RowScanner.Close()` gained an `error` return that its hand-rolled `pgxIntegQuerier` never adopted; `NewTokensHandler`'s signature gained a `*db.Pools` param) — blocking these new tests from running at all in this package. Replaced its stale duplicate querier with the already-correct `admin.NewPgxQuerier`. **Not fixed:** one test in that file, `TestIntegration_CreateToken_201`, still fails at runtime (not compile time) — `TokensHandler.Create` now requires tenant context via `tenantctx.MustTenantIDFromCtx`, which this old test's bare `httptest` request never sets up. Pre-existing, unrelated to this session's work, flagged rather than fixed (adding tenant-context middleware to that test harness is a separate task). | 2 |
...
| S2-14 | `them.llm_provider_keys` platform-owned rows (`tenant_id IS NULL`, db/108, applied live this session) against live Postgres (`internal/admin/dal/llm_provider_keys_integration_test.go`): `TestDAL_ProviderKey_PlatformOwned_CreateAndList`, `TestDAL_ProviderKey_PlatformOwned_InvisibleToTenantQueries` (a tenant-scoped query must never see a platform-owned key), `TestDAL_ProviderKey_PlatformOwned_NameUniqueAmongPlatformKeysOnly` (a tenant key with the same name as a platform key must succeed — uniqueness is scoped separately per the new partial indexes), `TestDAL_ProviderKey_PlatformOwned_SetDefault_ClearsPrevious` | 4 |
